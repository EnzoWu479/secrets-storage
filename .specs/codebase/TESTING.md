...
| API, store e componentes de gerenciamento de segredos | unitário frontend | `src/**/*.test.ts` | `pnpm test:frontend` | Paralelo seguro; usa a suíte frontend existente |
...
| Domínio Rust puro de gerenciamento de segredos | unitário | `src-tauri/src/secrets/**/*.rs` | `pnpm test:rust` | Paralelo seguro; usa a suíte Rust existente |
| Repository/serviço de gerenciamento de segredos com vault temporário | integração Rust | `src-tauri/tests/secret_management.rs` | `cargo test --manifest-path src-tauri/Cargo.toml --test secret_management -- --test-threads=1` | Serial; introduzido por T07 (comando indisponível até então) |
| Writer e clipboard Windows de gerenciamento de segredos | integração Windows | `src-tauri/tests/windows_secret_platform.rs` | `cargo test --manifest-path src-tauri/Cargo.toml --test windows_secret_platform -- --test-threads=1` | Serial; introduzido por T06 (comando indisponível até então) |
...
| Comandos e authority de gerenciamento de segredos | integração IPC | `src-tauri/tests/secret_management_commands.rs` | `cargo test --manifest-path src-tauri/Cargo.toml --test secret_management_commands -- --test-threads=1` | Serial; introduzido por T14 (comando indisponível até então) |
