...
**Current Work:** Backend Rust da feature `local-sessions` **implementado** — `session::{error,model,storage,manager,commands}` liga o núcleo `crypto::*` ao app via camada de comandos Tauri estreitos (16 comandos) + `SessionManager` (gate GMP, ciclo de sessões, storage em disco atômico, atraso progressivo, varredura de inatividade). Gate `pnpm check:rust` verde (**72 testes**: 53 cripto + 19 de sessão). Segue como **candidato** (D-05 e PT-01/PT-02 abertos). **Pendências desta feature:** rewire do frontend (`vault.ts`/`sessionsApi.ts` chamando `invoke` no lugar do placeholder inseguro de localStorage — AD-023), `rename_session` (exige primitivo de troca de nome no `crypto::envelope`), eventos de lock/suspend do Windows (PT-05). Backend cripto (`crypto-format`) segue completo como candidato; a revisão independente (`crypto-format/review-plan.md`) continua sendo o marco que destrava D-05.

---

## Recent Decisions (Last 60 days)

### AD-028: Backend Rust de `local-sessions` — SessionManager + comandos Tauri (2026-07-16)

**Decision:** Implementada a fatia **backend** da feature `local-sessions` (`.specs/features/local-sessions/tasks.md`, Fases 1–5): módulo `session` que orquestra o núcleo `crypto::*` já testado. **`session::error`** — `SessionError` tipado/serializável para o IPC, sem eco de segredo (falhas de autenticação colapsam em `Auth`; magic/versão/CBOR/params → `CorruptOrIncompatible`). **`session::model` + `storage`** — `Registry`/`SessionEntry`/`AuthMode`/`LockPolicy` com normalização de nome (unicidade case-insensitive) e escrita atômica (grava `.tmp` + rename) de `registry.json` (JSON legível bloqueado), `keyring.vault` e `vaults/<uuid>.vault` (CBOR via `ciborium`). **`SessionManager`** — estado gerenciado (`AppLock`/`unlocked`/`attempts`) com os 16 comandos do design: gate GMP (`app_status`/`create_global_password`/`unlock_app` abrindo todas as `global` de uma vez/`lock_app`/`change_global_password`) e ciclo de sessão (`list_sessions`/`create_session`/`unlock_session`/`lock_session`/`lock_all`/`change_master_password`/`set_session_auth_mode`/`set_lock_policy`/`touch_session`/`reveal_hint`/`delete_session`); atraso progressivo em memória (backoff 1→30s por chave global/sessão) e `sweep_locks` de inatividade. **`lib.rs`** — `manage(SessionManager)` + `invoke_handler`, relógio de inatividade (thread ~1s) e `lock_app` no `CloseRequested`. Gate `pnpm check:rust` verde (72 testes).
**Reason:** Dar prosseguimento ao roadmap ligando o backend cripto (AD-024…027) a uma superfície de app utilizável — o próximo passo de código adiado explicitamente para esta feature.
**Trade-off / desvios anotados:** (1) `created_at` gravado como `u64` (segundos Unix) em vez de ISO-8601 do design, evitando dependência de relógio formatado (o frontend formata). (2) `rename_session` **fora desta fatia**: honrar o nome autenticado na AAD do `.vault` exige um primitivo de troca de nome no `crypto::envelope` (o header autentica `session_name` e `rewrap` o preserva); renomear só no registry quebraria a verificação de integridade no `unlock`. (3) Estado sob um único `Mutex<Inner>` (mais simples e sem deadlock; o Argon2id roda com o lock preso — aceitável para app desktop de usuário único). (4) Senhas entram pelo IPC como `String`; zeroização dessas cópias fica para PT-04. (5) Eventos de lock/suspend do Windows são best-effort (validação profunda em PT-05). (6) Ambiente Linux exigiu libs GTK/WebKit do sistema para compilar o crate Tauri (não afeta o código).
**Impact / gate aberto:** A orquestração de app-unlock e o ciclo de sessões existem e são testados de ponta a ponta sobre o storage real. **Continua candidato**: não fecha D-05 nem fixa PT-01/PT-02. O frontend segue no placeholder inseguro (AD-023) até a fatia de rewire (`sessionsApi.ts` + `vault.ts` via `invoke`), que é a próxima pendência natural.
